...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        # print(list(zip(range(self.state_dim), self.mdp.states)))
        #
        # print(list(zip(range(self.action_dim), self.mdp.actions)))
        #
        # print(self.mdp.R)
        #
        # print(self.mdp.P)
        ...
        while True:
            ...
            for s in range(self.state_dim):
                v = V[s]
                V[s] = max([sum([self.mdp.P[s, a, s1] * (self.mdp.R[s, a, s1] + V[s1]) for s1 in range(self.state_dim)]) for a in range(self.action_dim)])
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for s in range(self.state_dim):
            best_action = np.argmax([sum([self.mdp.P[s, a, s1] * (self.mdp.R[s, a, s1] + self.gamma * V[s1])
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for s in range(self.state_dim):
                ...
                tmp = 0
                for a in range(self.action_dim):
                    tmp += policy[s, a] * sum([self.mdp.P[s, a, s1] * (self.mdp.R[s, a, s1] + self.gamma * V[s1]) for s1 in range(self.state_dim)])
                V[s] = tmp
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while not stable_state:
            ...
            for s in range(self.state_dim):
                ...
                best_action = np.argmax(np.array([np.sum(self.mdp.P[s, a, :] * (self.mdp.R[s, a, :] + self.gamma * V))
                                                  for a in range(self.action_dim)]))
