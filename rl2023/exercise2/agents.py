...
class QLearningAgent(Agent):
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        target = reward + self.gamma * max([self.q_table[(n_obs, a)] for a in range(self.n_acts)])
    ...
    def learn(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q2**
        ...
        for t in range(len(obses)):
            pair = (obses[t], actions[t])

            if pair not in sa_returns:
                sa_returns[pair] = []
            sa_returns[pair].append(sum([self.gamma ** (k - t) * rewards[k] for k in range(t, len(obses))]))

            if pair not in self.sa_counts:
                self.sa_counts[pair] = 0
            self.sa_counts[pair] += 1

            self.q_table[pair] += (1 / self.sa_counts[pair]) * (sum(sa_returns[pair]) - self.q_table[pair])

        updated_values = {k: self.q_table[k] for k in sa_returns.keys()}
